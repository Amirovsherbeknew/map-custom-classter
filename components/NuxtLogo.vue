<template><div>
  ...
  <input v-mask="'SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS'"  :tokens="mask" :maxlength="'10000'" v-model="value"/> {{ value }}
     <link href="https://cdn.jsdelivr.net/npm/tailwindcss@2.1.2/dist/tailwind.min.css" rel="stylesheet">
    <div id="hoverElement" class="max-w-4xl mx-auto sm:px-6 lg:px-8">
...
export default {
  ...
  mounted () {
    ...
    //   value?.focus()
    // }
    // // Hover (mouse over) hodisasi uchun event listener qo'shish
    // hoverElement.addEventListener('mouseover', function() {
    //   const speakText = hoverElement?.innerText
    //   const utterance = new SpeechSynthesisUtterance(speakText)
    //   speechSynthesis.speak(utterance)
    // });

    // // Hoverdan chiqish (mouse out) hodisasi uchun event listener qo'shish
    // hoverElement.addEventListener('mouseout', function() {
    //   if (speechSynthesis.speaking) {
    //     speechSynthesis.cancel(); // Ovoz to'xtatiladi
    //   }
    // });
  }
}
